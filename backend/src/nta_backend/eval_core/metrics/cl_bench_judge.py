...
@register_metric("cl_bench_acc")
class CLBenchJudgeMetric(Metric):
    def __init__(
        self,
        *,
        api_url: str,
    ...
    def score(self, sample: Sample, output: ModelOutput) -> SampleScore:
        ...
        overall_score = _normalize_binary_score(parsed.get("Overall Score"))
        return SampleScore(
            sample_id=sample.id,
            metric=self.name,
            score=float(overall_score),
            passed=bool(overall_score),
            reason=str(parsed.get("Grading Rationale") or ""),
            extra={
...
def _build_grading_prompt(
    ...
    *,
    ...
    if prompt_template:
        return (
            prompt_template.replace("{rubrics}", rubric_text).replace(
                "{student_response}",
                student_response,
            )
        )
    return (
        "You are a strict grader for CL-bench.\n"

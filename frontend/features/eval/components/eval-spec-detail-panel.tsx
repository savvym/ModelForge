          ...
          {files.map((file) => (
            <TableRow key={file.id}>
              <TableCell className="min-w-[220px] align-top">
                <div className="font-medium text-foreground">{file.display_name}</div>
                <div className="mt-1 text-xs text-muted-foreground">{file.file_key}</div>
                {file.file_name ? <div className="mt-2 text-xs text-muted-foreground">{file.file_name}</div> : null}
              </TableCell>
              <TableCell>
              ...
              <TableCell>
                ...
                {file.error_message ? (
                  <div className="mt-2 max-w-[220px] text-xs text-destructive">{file.error_message}</div>
                ) : null}
              </TableCell>
              ...
              <TableCell className="text-sm text-foreground">{formatBytes(file.size_bytes)}</TableCell>
              <TableCell className="text-sm text-foreground">{formatDateTime(file.last_synced_at)}</TableCell>
            </TableRow>
          ))}
        </TableBody>
      </Table>
  ...
  return (
    <div className="space-y-6">
      <section className="flex flex-wrap items-start justify-between gap-4">
        <div className="space-y-2">
          <div className="flex flex-wrap items-center gap-2">
            <h1 className="text-xl font-semibold text-foreground">{spec.display_name}</h1>
      ...
      <Card className="border-border bg-card/80 shadow-none">
        <CardHeader>
          <CardTitle className="text-base text-foreground">输入 / 输出契约</CardTitle>
        </CardHeader>
        <CardContent className="grid gap-4 xl:grid-cols-2">
          <div className="space-y-3">
            <div className="text-sm font-medium text-foreground">输入 Schema</div>
            <JsonBlock value={spec.input_schema_json} />
          </div>
          <div className="space-y-3">
            <div className="text-sm font-medium text-foreground">输出 Schema</div>
          ...
          >
            <CardHeader className="space-y-3">
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <div className="grid gap-2 text-right text-sm text-foreground">
            ...
            <CardContent className="space-y-5">
              <div className="space-y-3">
                <div className="text-sm font-medium text-foreground">数据集文件</div>
                <DatasetFileTable files={version.dataset_files} />
              </div>
              <div className="grid gap-4 xl:grid-cols-2">
                <div className="space-y-3">
                  <div className="text-sm font-medium text-foreground">执行配置</div>
                  <JsonBlock value={version.engine_config_json} />
                </div>
                <div className="space-y-3">
                  <div className="text-sm font-medium text-foreground">评分配置</div>
